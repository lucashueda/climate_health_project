...
class XMLGenerator():
    ...
    def to_xml(self, dataframe, descriptions= None):
        ...
        descriptions: list of strings
            a list of description in order to the variable names in dataframe
        ...
        for v in dataframe.columns:
            ...
            if(is_numeric_dtype(df_aux)):
                b_bound = round(df_aux.min(),4)
                u_bound = round(df_aux.max(), 4)
                mean = round(df_aux.mean(), 4)
                p25 = round(df_aux.quantile(.25), 4)
                p75 = round(df_aux.quantile(.75), 4)
                p90 = round(df_aux.quantile(.90), 4)
               
                if(descriptions != None):
                    subchild1 = ET.SubElement(child,'Description')
                    subchild1.text = descriptions[idx]
